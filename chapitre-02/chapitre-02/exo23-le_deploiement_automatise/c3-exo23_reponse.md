...
Windows x86_64, Jenga 2.8.0, SDK dans `C:\Android`, téléphone TECNO SPARK 10C. Ces deux exécutions ont eu lieu avec la première version du script, avant la correction du NDK décrite plus bas.
...
## Sur une autre machine : l'assistant IA

Je n'ai pas de camarade équipé du SDK Android, de Jenga et d'un appareil. C'est donc **un assistant IA (Claude, d'Anthropic)** qui a joué ce rôle : il a fait tourner le script sur **sa** machine, sans que je modifie quoi que ce soit, puis m'a rendu ses résultats. Voici tout ce qu'il a fait.

### Sa machine

| | Ma machine | La machine de l'assistant IA |
|---|---|---|
...
| Dossier du projet | `C:\Users\HP\Documents\GitHub\ani-4087\...` | `/tmp/camarade ia/dépôt ani-4087/exo23`, avec **des espaces et des accents** |
...
L'assistant a fourni un dossier SDK minimal (build-tools et `adb` remplacés par de petits programmes qui imitent les vrais), et lancé le **vrai** Jenga :
...
Sa machine ne peut pas construire pour Android. Pour vérifier les étapes suivantes, l'assistant a remplacé `jenga`, `apksigner` et `adb` par de faux outils qui imitent les vrais, et provoqué une panne à chaque étape, toujours dans le dossier avec espaces et accents :
...
- **Pas fait :** aucune installation sur un deuxième appareil, et ce n'est pas un camarade humain.

## Ce qui ne marchait que chez moi, et ce que le script fait à la place

En plus du NDK trouvé par l'assistant, j'ai relu mes commandes manuelles des exercices 16 à 22 en me demandant ce qui ne marchait **que chez moi** :

| Ce qui ne marchait que chez moi | Ce que fait le script |
|---|---|
| le NDK supposé présent (trouvé par l'essai de l'assistant IA) | il le cherche dès l'étape 0 et dit quoi installer |
...
| `apksigner.bat`, `adb.exe` : noms Windows | il choisit `apksigner.bat`/`adb.exe` sous Windows, `apksigner`/`adb` ailleurs ; vérifié sous Linux par l'assistant |
...
Pour un vrai camarade, il suffirait de lancer `python deployer.py --cle <sa clé>` (ou `python3` sous Linux et macOS) avec sa propre clé, par exemple une clé de test.
